...
final class APICaller {
    ...
    public func getAlbumDetails(for album: Album, completion: @escaping (Result<AlbumDetailModel, Error>) -> Void ) {
        createRequest(with:URL(
                        string: Constants.baseAPIURL + "/albums/" + album.id) ,
                      type: .GET
        )
        { request in
    ...
    public func getPlaylistDetails(for playlist: Playlist, completion: @escaping (Result<PlaylistDetailedModel, Error>) -> Void ) {
        createRequest(with:URL(
                        string: Constants.baseAPIURL + "/playlists/" + playlist.id) ,
                      type: .GET
        )
        { request in
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    let result = try JSONDecoder().decode(PlaylistDetailedModel.self, from: data)
                    completion(.success(result))
                }
                catch {
                    print(error)
    ...
    public func getCurrentUserPlaylists(completion: @escaping (Result<[Playlist], Error>) -> Void) {
        createRequest(with: URL(string: Constants.baseAPIURL + "/me/playlists/?limit=50"),
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    let result = try JSONDecoder().decode(LibaryPlaylistResponse.self, from: data)
                    completion(.success(result.items))
                }
                catch {
                    print(error)
    ...
    public func getRecomamendations(genres: Set<String>, completion: @escaping ((Result<RecomendationResponse, Error>)) -> Void) {
    ...
    public func getRecommendedGenres(completion: @escaping ((Result<RecomndedGenresResponse,Error>)) -> Void) {
    ...
    public func getCategorys(completion: @escaping (Result<[Category], Error>) -> Void) {
        createRequest(with: URL(string: Constants.baseAPIURL + "/browse/categories?limit=50"), type: .GET) { request in
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                ...
                do {
                    let result = try JSONDecoder().decode(AllCategoryResponse.self, from: data)
    ...
    public func getCategorysPlaylists(category: Category,completion: @escaping (Result<[Playlist], Error>) -> Void) {
        createRequest(with: URL(string: Constants.baseAPIURL + "/browse/categories/\(category.id)/playlists?limit=5"), type: .GET) { request in
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else{
                    completion(.failure(APIError.failedToGetData))
                    return
...
//            
//            print(request.url?.absoluteURL ?? "noone")
//            let task = URLSession.shared.dataTask(with: request) { data, _, error in
//                guard let data = data, error == nil else {
//                    completion(.failure(APIError.failedToGetData))
//                    return
